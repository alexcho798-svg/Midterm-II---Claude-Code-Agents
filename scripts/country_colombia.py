"""Generate a financial profile for Colombia from data/latam_finanzas_clean.csv."""

import pandas as pd

CSV_PATH = "C:/Users/moni0/OneDrive/Documentos/Midterm II - Claude Code & Agents/data/latam_finanzas_clean.csv"
COUNTRY = "Colombia"
...
def main():
    ...
    lines.append("### 1. Tamano de muestra y edad")
    lines.append(f"- Encuestados: {n}")
    lines.append(f"- Rango de edad: {age_min} - {age_max} anos\n")
    ...
    lines.append(f"- Desviacion estandar: {income.std():,.2f}\n")
    ...
    lines.append("### 6. Uso de IA y satisfaccion")
    lines.append(f"- Horas promedio de herramientas de IA por semana: {avg_ai_hours:.1f}")
    lines.append(f"- Satisfaccion financiera promedio (1-5): {avg_satisfaction:.2f}")
